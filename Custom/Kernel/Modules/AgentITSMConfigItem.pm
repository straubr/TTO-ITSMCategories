...
sub Run {
    ...
    $Self->{Category} = $ParamObject->GetParam( Param => 'Category' )
        || $Self->{AgentITSMConfigItemCategoryFilter}
        || 'All';
    ...
    # store category & filter (ClassID) in session
    $SessionObject->UpdateSessionID(
        SessionID => $Self->{SessionID},
        Key       => 'AgentITSMConfigItemCategoryFilter',
        Value     => $Self->{Category},
    );
    $SessionObject->UpdateSessionID(
        SessionID => $Self->{SessionID},
    ...
    $Output .= $LayoutObject->ITSMConfigItemListShow(
        ...
        Categories    => \%NavBarCategoryFilter,
        Filter        => $Self->{Filter},
